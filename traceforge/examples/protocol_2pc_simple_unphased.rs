...
impl Node {
    ...
    fn run_coordinator(&mut self, collect: &RoundCollector) {
        for _ in 0..self.num_rounds {
            ...
            self.broadcast_participants(&round, Message::Prepare(prepare));

            let votes = self.collect_votes(&round, collect, self.participants.len());
            ...
            self.broadcast_participants(&round, Message::Decide(decide));
        }
    }
    ...
    fn run_participant(&mut self, collect: &RoundCollector) {
        for _ in 0..self.num_rounds {
            ...
            comm_close::send(prepare.coordinator, Message::Vote(vote_msg), &round);

            let decide = self.collect_decide(&round, collect);
